...
describe('Admin Dashboard (e2e)', () => {
  ...
  describe('/api/admin/dashboard/metrics (GET)', () => {
    ...
    it('should return dashboard metrics with date range filter', () => {
      ...
      return request(app.getHttpServer())
        .get(`/api/admin/dashboard/metrics?date_from=${dateFrom}&date_to=${dateTo}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('total_users');
  ...
  describe('/api/admin/dashboard/sales-analytics (GET)', () => {
    it('should return sales analytics with required parameters', () => {
      ...
      return request(app.getHttpServer())
        .get(`/api/admin/dashboard/sales-analytics?date_from=${dateFrom}&date_to=${dateTo}`)
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(200)
        .expect((res) => {
    ...
    it('should return sales analytics with different intervals', async () => {
      ...
      for (const interval of intervals) {
        await request(app.getHttpServer())
          .get(`/api/admin/dashboard/sales-analytics?date_from=${dateFrom}&date_to=${dateTo}&interval=${interval}`)
          .set('Authorization', `Bearer ${adminToken}`)
          .expect(200)
          .expect((res) => {
            expect(res.body).toHaveProperty('sales_trend');
    ...
    it('should return 400 for invalid date format', () => {
      return request(app.getHttpServer())
        .get('/api/admin/dashboard/sales-analytics?date_from=invalid-date&date_to=2024-12-31')
        .set('Authorization', `Bearer ${adminToken}`)
        .expect(400);
    });
    ...
    it('should return 401 for non-admin user', () => {
      ...
      return request(app.getHttpServer())
        .get('/api/admin/dashboard/sales-analytics?date_from=2024-01-01&date_to=2024-12-31')
        .set('Authorization', `Bearer ${userToken}`)
        .expect(401);
    });
  });
