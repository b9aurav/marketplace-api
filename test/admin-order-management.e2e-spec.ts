...
describe("Admin Order Management (e2e)", () => {
  ...
  describe("GET /api/admin/orders", () => {
    ...
    it("should filter orders by status", async () => {
      ...
      expect(
        response.body.orders.every(
          (order: any) => order.status === OrderStatus.PENDING,
        ),
      ).toBe(true);
    });
    ...
    it("should filter orders by user_id", async () => {
      ...
      expect(
        response.body.orders.every(
          (order: any) => order.user.id === testUser.id,
        ),
      ).toBe(true);
    });
    ...
    it("should filter orders by amount range", async () => {
      ...
      expect(
        response.body.orders.every(
          (order: any) => order.total >= 50 && order.total <= 150,
        ),
      ).toBe(true);
    });
  ...
  describe("POST /api/admin/orders/:id/refund", () => {
    ...
    it("should process partial refund", async () => {
      ...
      expect(response.body.amount).toBe(25.0);
    });

    it("should return 400 for refund amount exceeding order total", async () => {
      const refundData = {
        amount: 150.0, // Exceeds order total
        reason: "Customer request",
      };
