...
describe('ProductsService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(Review), useValue: mockReviewsRepository },
      ],
    }).compile();
  ...
  describe('findAll', () => {
    it('should return paginated products with filters and sorting', async () => {
      ...
      const dto: FindProductsDto = { category: 'cat1', min_price: 10, max_price: 100, sort: SortOption.PRICE_ASC, page: 1, limit: 10 };
  ...
  describe('findOne', () => {
    it('should return a product with relations', async () => {
      ...
      expect(productsRepository.findOne).toHaveBeenCalledWith({ where: { id: '1' }, relations: ['reviews', 'reviews.user'] });
    });
  ...
  describe('getTrending', () => {
    it('should return top 10 products by rating', async () => {
      ...
      expect(productsRepository.find).toHaveBeenCalledWith({ order: { rating: 'DESC' }, take: 10 });
    });
  });

  describe('create', () => {
    it('should create and save a product', async () => {
      const dto: CreateProductDto = { name: 'P', description: 'D', price: 1, stock: 1, images: ['img'], category_id: 'cat' };
  ...
  describe('update', () => {
    it('should update and save a product', async () => {
      ...
      expect(productsRepository.save).toHaveBeenCalledWith({ ...mockProduct, ...dto });
    });
  });
}); 