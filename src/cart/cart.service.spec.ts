...
describe('CartService', () => {
  ...
  describe('getCart', () => {
    ...
    it('should create new cart if none exists', async () => {
      ...
      expect(cartRepository.create).toHaveBeenCalledWith({ user_id: 'user-uuid-1234' });
      expect(result).toHaveProperty('items');
      expect(result).toHaveProperty('total');
  ...
  describe('addItem', () => {
    ...
    it('should update existing item quantity', async () => {
      ...
      cartItemRepository.save.mockResolvedValue({ ...existingItem, quantity: 3 });
  ...
  describe('applyCoupon', () => {
    ...
    it('should throw BadRequestException for invalid coupon', async () => {
      ...
      await expect(
        service.applyCoupon('user-uuid-1234', 'A'),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('clearCart', () => {
    it('should clear all items and reset coupon', async () => {
      ...
      expect(cartItemRepository.delete).toHaveBeenCalledWith({ cart_id: mockCart.id });
      expect(cartRepository.save).toHaveBeenCalledWith({
        ...mockCart,
        coupon_code: null,
        discount_amount: 0,
