...
async function createAdminUser(
  ...
  if (!password) {
    password = await askQuestion(
      "Enter password for admin user (min 6 characters): ",
    );
  ...
  if (!name) {
    name =
      (await askQuestion(
        "Enter name for admin user (default: Admin User): ",
      )) || "Admin User";
  }
  ...
  try {
    ...
    console.log(
      `User with email ${email} not found. Creating new admin user...`,
    );
      ...
      if (
        ...
        try {
          ...
          if (existingSupabaseUser) {
            ...
            await userRepository.save(newUser);
            console.log(
              `Success: Admin user ${email} created in local database using existing Supabase account`,
            );
          } else {
          ...
          console.log(
            "3. The user can manually register first, then run this script to promote them",
          );
          process.exit(1);
        }
      } else {
