...
async function createAdminUser(email: string, password?: string, name?: string) {
  ...
  if (!password) {
    password = await askQuestion('Enter password for admin user (min 6 characters): ');
  ...
  if (!name) {
    name = await askQuestion('Enter name for admin user (default: Admin User): ') || 'Admin User';
  }
  ...
  try {
    ...
    try {
      ...
      console.log(`Success: User registered in Supabase with ID: ${supabaseUser.id}`);

      // Step 2: Create user in local database with admin role
      console.log('Creating user in local database...');
      const newUser = userRepository.create({
        id: supabaseUser.id, // Use Supabase user ID
        email: email,
        name: name,
        role: Role.ADMIN, // Create directly as admin
        is_active: true,
      });

      await userRepository.save(newUser);
      console.log(`Success: Admin user ${email} created in local database`);
      ...
      if (supabaseError.message?.includes('already registered') ||
        ...
        console.log('User already exists in Supabase. Attempting to create local database entry...');
        ...
        try {
          ...
          if (existingSupabaseUser) {
            ...
            await userRepository.save(newUser);
            console.log(`Success: Admin user ${email} created in local database using existing Supabase account`);
          } else {
            throw new Error('Could not retrieve existing Supabase user');
          }
        } catch (signInError) {
          console.error('Failed to sign in with existing Supabase account:', signInError.message);
          console.log('\n❌ Could not create admin user. Please ensure:');
          console.log('1. The email/password combination is correct if user exists in Supabase');
          console.log('2. Supabase is properly configured');
          console.log('3. The user can manually register first, then run this script to promote them');
          process.exit(1);
        }
      } else {
        console.log('\n❌ Failed to create admin user. Please ensure:');
