...
async function createUser(email: string, password?: string, name?: string) {
  ...
  if (!password) {
    password = await askQuestion('Enter password for user (min 6 characters): ');
  ...
  if (!name) {
    name = await askQuestion('Enter name for user (default: Test User): ') || 'Test User';
  }
      ...
      if (supabaseError.message?.includes('already registered') ||
        ...
        try {
          ...
          if (existingSupabaseUser) {
            ...
            await userRepository.save(newUser);
            console.log(`Success: User ${email} created in local database using existing Supabase account`);
          } else {
