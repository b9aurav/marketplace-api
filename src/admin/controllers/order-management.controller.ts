...
export class OrderManagementController {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve a paginated list of orders with comprehensive filtering options including search, status, user, payment method, amount range, and date range.",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve detailed information about a specific order including items, user, address, and tracking information.",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Update the status of an order with optional admin notes and tracking number. Status transitions are validated.",
  })
  @ApiParam({ name: "id", description: "Order ID" })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "Process a full or partial refund for an order through the payment gateway. Only eligible orders can be refunded.",
  })
  @ApiParam({ name: "id", description: "Order ID" })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: "Bad Request - Invalid refund amount or order not eligible",
  })
  @ApiResponse({ status: 401, description: "Unauthorized" })
  @ApiResponse({
    status: 403,
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve comprehensive order analytics including totals, trends, status distribution, payment methods, and growth metrics for a specified date range.",
  })
  @ApiResponse({
    status: 200,
  ...
  async getOrderAnalytics(
    @Query() query: OrderAnalyticsQueryDto,
