...
export class ProductManagementController {
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieve paginated list of products with advanced filtering and sorting options'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin access required' })
  @ApiQuery({ name: 'date_from', required: false, type: String, description: 'Filter from date (ISO string)' })
  @ApiQuery({ name: 'date_to', required: false, type: String, description: 'Filter to date (ISO string)' })
  ...
  async createProduct(@Body() data: AdminCreateProductDto): Promise<ProductDetailsDto> {
  ...
  @ApiOperation({ 
    ...
    description: 'Update stock quantity and minimum stock threshold for a product'
  })
  @ApiParam({ name: 'id', description: 'Product ID' })
  ...
  @ApiOperation({ 
    ...
    description: 'Perform bulk operations like activate, deactivate, delete, feature, or unfeature on multiple products'
  })
  @ApiResponse({ status: 200, description: 'Bulk action completed successfully' })
  @ApiResponse({ status: 400, description: 'Bad request - Invalid input data or some products not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Admin access required' })
  async bulkAction(@Body() data: BulkProductActionDto): Promise<{ message: string }> {
  ...
  @ApiOperation({ 
    ...
    description: 'Export products data in CSV or XLSX format with filtering options'
  })
  @ApiResponse({ 
    status: 202, 
  ...
  async exportProducts(@Body() data: ExportProductsDto): Promise<ExportResultDto> {
