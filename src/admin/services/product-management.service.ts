...
export class ProductManagementService {
  ...
  async getProductDetails(id: string): Promise<ProductDetailsDto> {
    ...
    // Cache the result
    await this.cacheService.set(cacheKey, productDto, CACHE_TTL.PRODUCT_DETAILS);
  ...
  async getProductAnalytics(dateFrom?: Date, dateTo?: Date): Promise<ProductAnalyticsDto> {
    const cacheKey = this.generateCacheKey(CACHE_KEYS.PRODUCT_ANALYTICS, { dateFrom, dateTo });
    ...
    const [
      ...
      queryBuilder.clone().andWhere('product.status = :status', { status: ProductStatus.ACTIVE }).getCount(),
      queryBuilder.clone().andWhere('product.status = :status', { status: ProductStatus.INACTIVE }).getCount(),
      queryBuilder.clone().andWhere('product.status = :status', { status: ProductStatus.DRAFT }).getCount(),
      queryBuilder.clone().andWhere('product.featured = :featured', { featured: true }).getCount(),
      queryBuilder.clone().andWhere('product.stock <= product.minimum_stock AND product.stock > 0').getCount(),
    ...
    const categoryDistributionWithPercentage = categoryDistribution.map(item => ({
      ...
      percentage: totalProducts > 0 ? (parseInt(item.product_count) / totalProducts) * 100 : 0
    }));

    // Get price distribution
    const priceDistribution = await Promise.all([
      queryBuilder.clone().andWhere('product.price < 100').getCount(),
      queryBuilder.clone().andWhere('product.price >= 100 AND product.price < 500').getCount(),
      queryBuilder.clone().andWhere('product.price >= 500 AND product.price < 1000').getCount(),
    ...
    const analytics: ProductAnalyticsDto = {
      ...
      total_inventory_value: parseFloat(inventoryStats.total_inventory_value) || 0,
    ...
    // Cache the result
    await this.cacheService.set(cacheKey, analytics, CACHE_TTL.PRODUCT_ANALYTICS);
  ...
  private async transformToProductDetailsDto(product: Product): Promise<ProductDetailsDto> {
    return {
      id: product.id,
      name: product.name,
