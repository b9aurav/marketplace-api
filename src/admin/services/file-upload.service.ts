...
export class FileUploadService {
    ...
    async uploadImage(
        ...
        try {
            ...
            try {
                ...
                this.logger.warn(`Failed to cleanup file after error: ${cleanupError.message}`);
            }
    ...
    async getFiles(query: GetFilesQueryDto): Promise<PaginatedFilesResponseDto> {
        ...
        const cached = await this.cacheService.get<PaginatedFilesResponseDto>(cacheKey);
        if (cached) {
            return cached;
        }
    ...
    private async processImage(buffer: Buffer, mimeType: string): Promise<Buffer> {
        ...
        try {
            ...
            this.logger.warn(`Failed to process image, using original: ${error.message}`);
            return buffer;
        }
    }
