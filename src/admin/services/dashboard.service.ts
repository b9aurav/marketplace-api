...
export class DashboardService {
  ...
  async getDashboardMetrics(query: GetDashboardMetricsQueryDto): Promise<DashboardMetricsDto> {
    const cacheKey = this.generateCacheKey('dashboard:metrics', query);
    
    // Try to get from cache first
    const cachedData = await this.cacheService.get<DashboardMetricsDto>(cacheKey);
    if (cachedData) {
    ...
    const previousPeriodMetrics = await this.calculatePreviousPeriodMetrics(dateFrom, dateTo);
    ...
    const metrics: DashboardMetricsDto = {
      ...
      user_growth: this.calculateGrowthRate(currentMetrics.totalUsers, previousPeriodMetrics.totalUsers),
      order_growth: this.calculateGrowthRate(currentMetrics.totalOrders, previousPeriodMetrics.totalOrders),
      revenue_growth: this.calculateGrowthRate(currentMetrics.totalRevenue, previousPeriodMetrics.totalRevenue),
  ...
  async getSalesAnalytics(query: GetSalesAnalyticsQueryDto): Promise<SalesAnalyticsDto> {
    ...
    const salesTrend = await this.calculateSalesTrend(dateFrom, dateTo, interval);
    ...
    const previousMetrics = await this.calculateSalesMetrics(previousDateFrom, previousDateTo);
    ...
    const analytics: SalesAnalyticsDto = {
      ...
      growth_rate: this.calculateGrowthRate(totalMetrics.totalRevenue, previousMetrics.totalRevenue),
      average_order_value: totalMetrics.totalOrders > 0 ? totalMetrics.totalRevenue / totalMetrics.totalOrders : 0,
  ...
  private async calculateMetrics(dateFrom?: Date, dateTo?: Date) {
    ...
    const revenueQueryBuilder = this.orderRepository.createQueryBuilder('order');

    if (dateFrom && dateTo) {
      userQueryBuilder.where('user.created_at BETWEEN :dateFrom AND :dateTo', { dateFrom, dateTo });
      orderQueryBuilder.where('order.created_at BETWEEN :dateFrom AND :dateTo', { dateFrom, dateTo });
      revenueQueryBuilder.where('order.created_at BETWEEN :dateFrom AND :dateTo', { dateFrom, dateTo });
    }
    ...
    const [
      ...
      this.userRepository
        ...
        .andWhere(dateFrom && dateTo ? 'user.created_at BETWEEN :dateFrom AND :dateTo' : '1=1', 
          dateFrom && dateTo ? { dateFrom, dateTo } : {})
        .getCount(),
      this.orderRepository
        .createQueryBuilder('order')
        .where('order.status = :status', { status: OrderStatus.PENDING })
        .andWhere(dateFrom && dateTo ? 'order.created_at BETWEEN :dateFrom AND :dateTo' : '1=1',
          dateFrom && dateTo ? { dateFrom, dateTo } : {})
        .getCount(),
  ...
  private async calculateSalesTrend(dateFrom: Date, dateTo: Date, interval: string): Promise<SalesTrendItem[]> {
  ...
  private getDateGroupBy(interval: string): string {
    ...
    switch (interval) {
      case 'week':
        return dbType === 'postgres' ? "TO_CHAR(order.created_at, 'YYYY-\"W\"WW')" : "STRFTIME('%Y-W%W', order.created_at)";
      case 'month':
        return dbType === 'postgres' ? "TO_CHAR(order.created_at, 'YYYY-MM')" : "STRFTIME('%Y-%m', order.created_at)";
      default:
        return dbType === 'postgres' ? "TO_CHAR(order.created_at, 'YYYY-MM-DD')" : "STRFTIME('%Y-%m-%d', order.created_at)";
    }
  }
